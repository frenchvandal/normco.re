...
import { toStoryData, type StoryData, renderPostListItem } from "./utils/story-data.ts";
...
function renderTopicList(
  ...
  const items = tags.map((tag) =>
    `<li class="${classes.item}">
      <a href="${escapeHtml(getTagUrl(tag, language))}" class="${classes.link}">${escapeHtml(tag)}</a>
    </li>`
  ).join("\n");
...
function renderFeaturedStory(story: StoryData, language: SiteLanguage): string {
  ...
  <div class="primer-home-featured-story__body">
    ...
    <h2 class="primer-home-featured-story__title">
      <a class="primer-home-featured-story__link" href="${escapeHtml(story.url)}">${escapeHtml(story.title)}</a>
    </h2>
    ${story.summary === undefined ? "" : `<p class="primer-home-featured-story__summary">${escapeHtml(story.summary)}</p>`}
    <div class="primer-home-featured-story__meta">
      <time datetime="${escapeHtml(story.dateIso)}">${escapeHtml(story.dateLabel)}</time>${
    story.readingLabel === undefined
      ? ""
      : `<span class="primer-home-featured-story__reading">${escapeHtml(story.readingLabel)}</span>`
  }
    </div>
  </div>
</article>`;
...
function renderEmptyState(aboutUrl: string, t: SiteTranslations): string {
  return `<div class="primer-home-empty-state">
    ...
    <a href="${escapeHtml(aboutUrl)}" class="btn btn-sm">${escapeHtml(t.navigation.about)}</a>
  </div>`;
}
...
export default async (
  ...
  const featuredTags = distinct(stories.flatMap((s) => [...s.tags])).slice(0, 4);
  ...
  const listingMarkup = (await Promise.all(rest.map((story) =>
  ...
  <div class="primer-home-section-head">
    <div class="primer-home-section-head-copy">
      ...
      <h2 id="home-recent-title" class="primer-home-section-title">${escapeHtml(t.home.recentHeading)}</h2>
    </div>
    <a href="${escapeHtml(archiveUrl)}" class="primer-home-section-link">${escapeHtml(t.home.archiveLinkLabel)}</a>
  </div>
  ${
      recent.length === 0
    ...
    ${featuredStory}
    ${listingMarkup.length > 0 ? `<ul class="home-posts home-posts--ledger">${listingMarkup}</ul>` : ""}
  </div>`
    }
</section>`;
...
<section class="primer-home-intro" aria-labelledby="home-title">
  ...
  <div class="primer-home-intro__grid">
    ...
    <div class="primer-home-intro__aside">
      <nav class="primer-home-intro__links" aria-label="${escapeHtml(t.site.mainNavigationAriaLabel)}">
        <a href="${escapeHtml(archiveUrl)}" class="primer-home-inline-link primer-home-inline-link--primary">${escapeHtml(t.home.archiveLinkLabel)}</a>
        <a href="${escapeHtml(aboutUrl)}" class="primer-home-inline-link">${escapeHtml(t.navigation.about)}</a>
      </nav>
      ${introTopicMarkup.length === 0 ? "" : `<div class="primer-home-intro__topics">${introTopicMarkup}</div>`}
    </div>
  </div>
</section>
