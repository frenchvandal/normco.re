...
describe("feed-paths", () => {
  it("exposes the canonical feed paths", () => {
    assertStrictEquals(RSS_FEED_PATH, "/rss.xml");
    assertStrictEquals(ATOM_FEED_PATH, "/atom.xml");
  ...
  it("localizes feed URLs through the shared URL helper", () => {
    assertStrictEquals(getLocalizedRssFeedUrl("en"), "/rss.xml");
    assertStrictEquals(getLocalizedAtomFeedUrl("fr"), "/fr/atom.xml");
    assertStrictEquals(
