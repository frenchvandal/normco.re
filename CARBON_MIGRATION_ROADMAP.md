...
goal: adopt Carbon tokens, patterns, and high-value behavior where they
materially reduce long-term custom maintenance, while preferring native HTML +
CSS for low-behavior surfaces that do not benefit from a custom-element runtime.

The plan intentionally combines findings from:

- `carbon/deep-research-report.md`
- `carbon/Systèmes de Design Open Source et Frameworks.md`
...
| Phase 3 - Content surfaces migration               | In progress | Content surfaces are now being rebalanced under a token-first strategy: low-ROI editorial primitives are moving back to native semantic HTML, while behavior-heavy affordances such as code copy remain selectively Carbon-backed when justified.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| Phase 5 - Search, media, and performance hardening | In progress | Slices 1 to 22 are completed: search scope/mount parity is aligned across main/feed/sitemap, editorial image-dimension gating is enforced and tested, payload reporting now includes PR-friendly regression guards plus a versioned policy mode with explicit route-existence, baseline route-parity, and metadata coherence validation for policy version/fingerprint/policy-run marker/policy-baseline provenance marker/policy-baseline completeness fields on baseline and current policy reports, along with a dedicated policy-compatible baseline generation flow for CI/PR usage; P5-S17 fixed the critical home-navigation regression, P5-S18 hardened static reliability with an explicit browser-import guard in build output, P5-S19 removed runtime CDN imports by self-hosting Carbon component modules in build output, P5-S20 started the token-first pivot by removing low-ROI editorial Carbon primitives from runtime bootstrap and rendered markup, P5-S21 removed cds-button from runtime bootstrap, and P5-S22 replaced cds-switcher with a native &lt;select&gt; element styled with Carbon tokens. |
...
- For a Lume + TSX static site, `@carbon/web-components` is appropriate for
  behavior-heavy shells, but it is not the end goal for every migrated surface;
  native semantic HTML plus Carbon-aligned tokens is preferred when behavior is
  simple.
...
- Large global CSS utility layers should not keep growing during migration; move
  styling responsibility into Carbon tokens/patterns first, and only keep a
  minimal local adaptation layer around native or Carbon-backed primitives.

## 2.3 Decision statement

- Carbon tokens, spacing, and interaction patterns become the default design
  source of truth.
- Local TSX components remain composition shells and keep business/i18n logic.
- Native semantic HTML is preferred for low-behavior surfaces, even when Carbon
  offers a matching component.
- Carbon Web Components stay selectively allowed for behavior-heavy shells and
  affordances, with parity checkpoints after each slice.
- Carbon-aligned defaults come first; custom CSS/JS is exception-only and must
  be justified, temporary, and tracked.
...
- Add `@carbon/web-components` via `deno.json` npm alias only for slices that
  need behavior-heavy Carbon elements.
- Create a dedicated client bootstrap module (for example
  `src/scripts/carbon.js`) with selective side-effect imports only for the
  custom elements that still justify runtime registration.
- Load this bootstrap from the base layout before those Carbon elements are
  expected to render interactively.
...
- Migrate internals to Carbon-aligned tokens/patterns first, then to `<cds-*>`
  only when Carbon behavior materially improves the surface.
...
- Use Carbon tokens, spacing, and interaction patterns as-is by default.
- Use Carbon Web Components only when they provide meaningful accessibility or
  interaction value beyond native semantic HTML.
...
- Prefer native semantic HTML over new bespoke primitives for low-behavior
  surfaces, even if Carbon exposes an equivalent web component.
...
## 3.5 P5-S20 token-first audit outcome

| Bucket  | Components                                                     | Decision | Why                                                                                                                                           |
| ------- | -------------------------------------------------------------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| Keep    | `cds-header*`, `cds-side-nav*`, `cds-switcher*`, `cds-button`  | Keep     | These still carry the highest interaction complexity and currently centralize shell behavior with less risk than a broad rewrite.             |
| Replace | `cds-link`, `cds-tag`, `cds-breadcrumb`, `cds-breadcrumb-item` | Replace  | These editorial primitives add runtime/bootstrap weight without unique behavior that native semantic HTML cannot cover more robustly.         |
| Defer   | `cds-copy-button`                                              | Defer    | It still provides behavior value on post code blocks, but should be re-evaluated only after the `[Carbon-P3]` fallback carryover is resolved. |
...
3. Update contributor guidance files (`AGENTS.md`, `CLAUDE.md`, `README.md`)
   with Carbon token-first conventions and selective behavior-heavy import
   policy.
...
7. Explicitly remove the previously deferred contact channel (Cloudflare Worker
   -> Telegram relay) from Carbon migration scope; track it separately as a
   product backlog decision, not as a migration deliverable.
...
- Deferred contact channel is explicitly marked as out of scope for this
  migration, with no remaining migration blocker tied to contact-flow delivery.
...
| ---------- | ------- | ----------- | ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-03-13 | P5-S22  | Completed   | Working tree | Delivered token-first slice by replacing `cds-switcher` + `cds-switcher-item` Carbon web components with native `<select>` for language selection: extracted Carbon design tokens from `/design/*.json` (Colors.json: Gray scale for hover/focus states, Spacing.json: `spacing-03`/`spacing-05`/`spacing-06` for padding and touch target), implemented native semantic HTML select with Carbon-aligned CSS in `src/styles/layout.css` using `appearance: base-select` + `::picker(select)` for native dropdown styling (48px min-size, focus-visible ring, hover transition, checked option highlight), updated `src/_components/Header.tsx` to use native select with onchange language routing while preserving `getLocalizedUrl` logic and all ARIA labels, removed `cds-switcher` and `cds-switcher-item` from `src/scripts/carbon.js` selective bootstrap (10 entries from 12), and added rendering contract tests in `src/_components/Header_test.ts` for select options/selected state plus bootstrap validation in `src/scripts/carbon_test.ts` to prevent reintroduction.                                             | 2 TODOs (accepted carryover): `src/scripts/post-code-copy.js` (`[Carbon-P3]` remove `execCommand` fallback after clipboard API support baseline for site visitors reaches parity in analytics), `src/styles/components.css` (`[Carbon-P5]` remove footer utility-link token overrides after global Carbon link tokens preserve muted idle/hover affordance parity on home, feed, and sitemap without local wrappers). | Scripts (migrated chrome): 7 files, 1,103 LOC (no change vs P5-S21, +243 vs baseline). Selectors (search/content surfaces): replaced 2 Carbon runtime selectors (`cds-switcher`, `cds-switcher-item`) with native equivalent while adding 6 new CSS rules for `.site-language-select` in layout.css. Payload (default key routes): total JS/CSS bytes delta 0 vs P5-S22 baseline report (no new JS/CSS bytes, Carbon vendor bundle reduced by 2 entries). |
...
| 2026-03-13 | P5-S20  | Completed   | Working tree | Pivoted Carbon migration strategy from web-components-first to token-first: formalized Carbon as design/tokens reference while preferring native semantic HTML + CSS for low-behavior surfaces; delivered first slice by removing `[Carbon-P5]` footer utility-link overrides from `src/styles/components.css`, adding explicit hover/focus-visible styles in `src/styles/layout.css` using local tokens, removing duplicate entries from `src/scripts/carbon.js` selective bootstrap (13 entries from 15), and adding CSS contract tests in `src/_components/Footer_test.ts` to lock hover/focus presence; updated `src/scripts/carbon_test.ts` to reflect cleaned entry count.                                                                                                                                                                                                                                                                                                                                                                                                                                                 | 2 TODOs (accepted carryover): `src/scripts/post-code-copy.js` (`[Carbon-P3]` remove `execCommand` fallback after clipboard API support baseline for site visitors reaches parity in analytics), `src/styles/components.css` (`[Carbon-P5]` remove footer utility-link token overrides after global Carbon link tokens preserve muted idle/hover affordance parity on home, feed, and sitemap without local wrappers). | Scripts (migrated chrome): 7 files, 1,103 LOC (down 4 vs P5-S19, +243 vs baseline). Selectors (search/content surfaces): replaced 4 low-ROI Carbon runtime selectors with native equivalents while keeping selector debt flat overall. Payload (default key routes): total JS/CSS bytes delta 0 vs P5-S20 baseline report (CSS-only change, no JS impact).                                                                                                |
| 2026-03-13 | P5-S19  | Completed   | Working tree | Prioritized front reliability by removing runtime CDN dependency from Carbon bootstrap: `src/scripts/carbon.js` now resolves selective component imports to self-hosted local bundle entries under `/scripts/carbon-vendor/`, while preserving Carbon-by-default selective registration and static-hosting compatibility; added a lightweight vendor build step (`scripts/build-carbon-vendor.ts`) that generates local Carbon browser bundles from npm sources via `deno bundle --platform browser --code-splitting` during `afterBuild` and covered deterministic entry mapping/collision behavior in `scripts/build-carbon-vendor_test.ts`; hardened shipped-import validation by extending `scripts/check-browser-imports.ts` to scan scripts recursively and fail explicitly on network import specifiers (`http://`, `https://`) in addition to existing `npm/`, `jsr:`, `node:`, and unresolved bare specifiers, with updated pass/fail coverage in `scripts/check-browser-imports_test.ts`; and updated `src/scripts/carbon_test.ts` to lock the new self-hosted runtime contract and prevent remote import regressions. | 2 TODOs (accepted carryover): `src/scripts/post-code-copy.js` (`[Carbon-P3]` remove `execCommand` fallback after clipboard API support baseline for site visitors reaches parity in analytics), `src/styles/components.css` (`[Carbon-P5]` remove footer utility-link token overrides after global Carbon link tokens preserve muted idle/hover affordance parity on home, feed, and sitemap without local wrappers). | Scripts (migrated chrome): 7 files, 1,107 LOC (up 1 vs P5-S18, +247 vs baseline). Selectors (search/content surfaces): no new custom selector debt introduced in this slice (no change vs P5-S18). Payload (default key routes): total JS/CSS bytes unchanged (delta 0) vs P5-S19 baseline report.                                                                                                                                                        |
| 2026-03-13 | P5-S18  | Completed   | Working tree | Fixed the remaining critical home UX regression by switching `src/scripts/carbon.js` from jsDelivr module URLs (which left unresolved bare dependencies in browsers) to browser-resolvable Carbon module URLs via unpkg `?module`, restoring custom-element registration for interactive links/panels without altering reading-first layout contracts; added targeted runtime coverage in `src/scripts/carbon_test.ts` to lock the browser-resolvable URL contract; introduced a lightweight shipped-output guard (`scripts/check-browser-imports.ts`) and coverage (`scripts/check-browser-imports_test.ts`) that fails builds on forbidden `npm/`, `jsr:`, `node:`, bare specifiers, or non-literal dynamic imports outside approved Carbon bootstrap handling; and wired the guard into `_config.ts` `afterBuild` so `deno task build` blocks non-browser-resolvable imports before merge.                                                                                                                                                                                                                                    | 2 TODOs (accepted carryover): `src/scripts/post-code-copy.js` (`[Carbon-P3]` remove `execCommand` fallback after clipboard API support baseline for site visitors reaches parity in analytics), `src/styles/components.css` (`[Carbon-P5]` remove footer utility-link token overrides after global Carbon link tokens preserve muted idle/hover affordance parity on home, feed, and sitemap without local wrappers). | Scripts (migrated chrome): 7 files, 1,106 LOC (no change vs P5-S17, +246 vs baseline). Selectors (search/content surfaces): no new custom selector debt introduced in this slice (no change vs P5-S17). Payload (default key routes): total JS/CSS bytes reduced by 28 vs P5-S18 baseline report.                                                                                                                                                         |
