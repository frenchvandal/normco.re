...
| Phase 5 - Search, media, and performance hardening | In progress | Slices 1 to 10 are completed: search scope/mount parity is aligned across main/feed/sitemap, editorial image-dimension gating is enforced and tested, payload reporting now includes PR-friendly regression guards plus a versioned policy mode with explicit route-existence, baseline route-parity, and metadata coherence validation for CI, and incremental route-level payload reductions were delivered without UX regressions. |
