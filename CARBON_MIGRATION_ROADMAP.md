...
| Phase 5 - Search, media, and performance hardening | In progress | Slices 1 to 14 are completed: search scope/mount parity is aligned across main/feed/sitemap, editorial image-dimension gating is enforced and tested, payload reporting now includes PR-friendly regression guards plus a versioned policy mode with explicit route-existence, baseline route-parity, and metadata coherence validation for policy version/fingerprint/policy-run marker/policy-baseline provenance marker, along with a dedicated policy-compatible baseline generation flow for CI/PR usage, while incremental route-level payload reductions continue without UX regressions. |
